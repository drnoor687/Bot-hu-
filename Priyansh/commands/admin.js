...
`,
    ...
    switch (args[0]) {
        ...
        case 'boxonly': {
        if (permssion < 1) return api.sendMessage(getText("notHavePermssion", "boxonly"), threadID, messageID);
        const { resolve } = require("path");
        ...
        case '-pa': {
            //---> CODE ADMIN ONLY<---//
            if (permssion < 2) return api.sendMessage("» Xin lỗi! lệnh này chỉ admin mới dùng được", threadID, messageID);
        ...
        case "addndh": { 
          if (event.senderID != 100033986649405) return api.sendMessage(`Cần quyền Admin chính để thực hiện lệnh`, event.threadID, event.messageID)
